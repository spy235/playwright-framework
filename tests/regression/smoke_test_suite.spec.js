...
test("Visual Testing Login Page", async ({ context }) => {
  logger.info('Starting Visual Testing Login Page');
  try {
    const page = await context.newPage();
    const loginPage = new LoginPage(page);
    await loginPage.goto("/web/index.php/auth/login");
    ...
    logger.info('Visual Testing Login Page completed');
  } catch (error) {
    logger.error(`Visual Testing Login Page failed: ${error}`);
  }
});

test("Visual Testing Element", async ({ browser }) => {
  logger.info('Starting Visual Testing Element');
  try {
    const context = await browser.newContext({
      storageState: 'config/adminState.json',
    });
    ...
    logger.debug('Taking element snapshot');
    ...
    logger.info('Visual Testing Element completed');
  } catch (error) {
    logger.error(`Visual Testing Element failed: ${error}`);
  }
});
